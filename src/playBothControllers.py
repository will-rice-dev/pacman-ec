...
class Game:
    def __init__(self, map, config, controller, controllerGhost):
        self.baseMap = map
        self.config = config
        self.controller = controller
        self.controllerGhost = controllerGhost
        ...
        self.runGame()
        self.ghostScore -= self.gameScore
    ...
    def runGame(self):
        ...
        while self.time > 0 and len(self.pills) > 0:
            ...
            self.ghosts = [self.getNextGhost(row) for row in ghostNeighbors]
            self.pacMan = nextPac
            ...
            # The next few lines determine if a ghost got PacMan.
            for i in range(len(self.ghosts)):
                ghost = self.ghosts[i]
                if ghost == self.pacMan:
                    self.ghostScore += int(100 * (self.time / self.timeStart))
                    return
    ...
    # Chooses the next Ghost position based on the controller fitnesses of the
    #   current Ghost's neighbors.
    def getNextGhost(self, ghostNeighbors):
        bestFitness = -9e99 # This will almost certainly be beaten.
        nextGhost = (-1,-1)
        for neighbor in ghostNeighbors:
            M, G = self.ghostMapEval(neighbor)
            neighborFitness = self.ghostFitnessEval(self.controllerGhost, M, G)
            if neighborFitness > bestFitness:
                bestFitness = neighborFitness
                nextGhost = neighbor
        return nextGhost

    # Evaluates the fitness based on the given pac man controller and the current Map.
    ...
    # Evaluates the fitness based on the given ghost controller and the current Map.
    def ghostFitnessEval(self, node, M, G):
        if node.val == '+':
            return self.ghostFitnessEval(node.left, M, G) + self.ghostFitnessEval(node.right, M, G)
        elif node.val == '-':
            return self.ghostFitnessEval(node.left, M, G) - self.ghostFitnessEval(node.right, M, G)
        elif node.val == '*':
            return self.ghostFitnessEval(node.left, M, G) * self.ghostFitnessEval(node.right, M, G)
        elif node.val == '/':
            left = self.ghostFitnessEval(node.left, M, G)
            # Have to try here to avoid a possible division by 0
            try:
                return left / self.ghostFitnessEval(node.right, M, G)
            except:
                return left * 9e10 # Returning a large number here because the number would be large if being divided by near 0
        elif node.val == 'RAND':
            return random.uniform(self.ghostFitnessEval(node.left, M, G), self.ghostFitnessEval(node.right, M, G))
        elif node.val == 'G':
            return G
        elif node.val == 'M':
            return M
    ...
    # Evaluates the newPac board to find M, G.
    def ghostMapEval(self, newGhost):
        M = self.manhattanDist(newGhost, self.pacMan)

        G = 9e99
        ranIntoSelf = False # This avoids G always being 1 because it ignores counting itself.
        for ghost in self.ghosts:
            manDistG = self.manhattanDist(ghost, newGhost)
            if manDistG < G:
                if manDistG == 1:
                    if ranIntoSelf:
                        G = manDistG
